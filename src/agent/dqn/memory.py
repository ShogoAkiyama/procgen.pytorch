...
class LazyMemory(dict):
    ...
    def __init__(self, capacity, batch_size, state_shape, device):
    ...
    def sample(self):
        indices = np.random.randint(low=0, high=len(self), size=self.batch_size)
    ...
    def _sample(self, indices):
        ...
        states = np.empty(
            (self.batch_size, *self.state_shape), dtype=np.uint8)
...
class LazyMultiStepMemory(LazyMemory):
    ...
    def __init__(self, capacity, batch_size, state_shape, device, gamma=0.99,
                 multi_step=3):
        super(LazyMultiStepMemory, self).__init__(
            capacity, batch_size, state_shape, device)
